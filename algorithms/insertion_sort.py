...
def insertion_sort(arr, time_limit=None):
    comparisons = 0
    ...
    for i in range(1, len(arr)):
        ...
        while j >= 0 and key < arr[j]:
            if time_limit is not None and (time.time() - start_time) > time_limit:
                logger.info("Tempo limite atingido, interrompendo Insertion Sort")
        ...
        arr[j + 1] = key
        comparisons += 1  # Comparação final quando key >= arr[j]
...
if __name__ == "__main__":
    ...
    for case_name, test_array in test_cases:
        ...
        sorted_array, comparisons = insertion_sort(test_array.copy(), time_limit=1)
