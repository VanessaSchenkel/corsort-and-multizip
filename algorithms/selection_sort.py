...
def selection_sort(arr, time_limit=None):
    comparisons = 0
    start_time = time.time()

    for i in range(len(arr)):
        if time_limit and (time.time() - start_time) >= time_limit:
            logger.info("Tempo limite atingido durante o Selection Sort")
...
if __name__ == "__main__":
    ...
    for case_name, test_array in test_cases:
        ...
        sorted_array, comparisons = selection_sort(test_array.copy(), time_limit=1)
